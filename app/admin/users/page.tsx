...
export default function UsersPage() {
    ...
    try {
      const response = await fetch('/api/admin/users', {
        ...
        body: JSON.stringify({
          ...
          ...(currentAction === 'suspend' && suspendReason ? { reason: suspendReason } : {}),
        }),
      });
      ...
      if (response.ok) {
        setToast({ open: true, message: result.message || 'Action completed', severity: 'success' });
  ...
  const handleInviteAdmin = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setToast({ open: true, message: result.message || 'Invitation sent', severity: 'success' });
        setInviteDialogOpen(false);
        setInviteEmail('');
      } else {
        setToast({ open: true, message: result.error || 'Failed to send invite', severity: 'error' });
      }
    } catch {
      setToast({ open: true, message: 'Network error', severity: 'error' });
    } finally {
  ...
  const getActionTitle = (action: string | null) => {
    switch (action) {
      case 'toggle_admin': return 'Toggle Admin Status';
  ...
  const getActionMessage = (action: string | null) => {
    switch (action) {
      case 'toggle_admin': return 'This will change the admin status for this user.';
      case 'suspend': return 'The user account will be suspended and they will receive a notification email.';
      case 'activate': return 'The user account will be reactivated.';
      case 'delete': return 'This will permanently delete the user and all their data (OAuth apps, tokens, identities). This action cannot be undone.';
    ...
    <Box>
            ...
            <Table>
              <TableHead>
                <TableRow sx={{ bgcolor: '#1a201c', borderBottom: '1px solid #333' }}>
                  ...
                  <TableCell sx={{ color: '#9ca3af', fontWeight: 600 }}>Verified</TableCell>
                  <TableCell sx={{ color: '#9ca3af', fontWeight: 600 }}>Apps</TableCell>
                  <TableCell sx={{ color: '#9ca3af', fontWeight: 600 }}>Last Login</TableCell>
                  <TableCell sx={{ color: '#9ca3af', fontWeight: 600 }}>Actions</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {data?.users.map((user) => (
                  <TableRow key={user.id} sx={{ borderBottom: '1px solid #333', '&:hover': { bgcolor: 'rgba(34, 197, 94, 0.05)' } }}>
                    <TableCell sx={{ color: '#e5e7eb' }}>
                      <Box>
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>{user.email}</Typography>
                        <Typography variant="caption" sx={{ color: '#6b7280' }}>{user.id}</Typography>
                      </Box>
                    </TableCell>
                    <TableCell>
                      <Chip
                        ...
                        size="small"
                        sx={{
                          bgcolor: user.isActive ? 'rgba(34, 197, 94, 0.2)' : 'rgba(239, 68, 68, 0.2)',
                    ...
                    <TableCell>
                      <Chip
                        ...
                        size="small"
                        sx={{
                          bgcolor: user.isAdmin ? 'rgba(59, 130, 246, 0.2)' : 'rgba(107, 114, 128, 0.2)',
                    ...
                    <TableCell>
                      <Chip
                        ...
                        variant="outlined"
                        sx={{
                          borderColor: user.emailVerified ? '#22c55e' : '#6b7280',
                          color: user.emailVerified ? '#22c55e' : '#9ca3af',
                        }}
                      />
                    </TableCell>
                    <TableCell sx={{ color: '#e5e7eb' }}>{user.appsCount}</TableCell>
                    <TableCell sx={{ color: '#9ca3af' }}>
                      {user.lastLogin ? new Date(user.lastLogin).toLocaleDateString() : 'Never'}
                    </TableCell>
                    <TableCell>
                      <IconButton size="small" onClick={(e) => handleMenuOpen(e, user.id)} sx={{ color: '#9ca3af' }}>
      ...
      {data && data.pagination.pages > 1 && (
        <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1, mt: 2 }}>
          <Button disabled={page <= 1} onClick={() => setPage(page - 1)} sx={{ color: '#9ca3af', textTransform: 'none' }}>Previous</Button>
          <Typography sx={{ color: '#9ca3af', alignSelf: 'center', fontSize: '0.9rem' }}>
            Page {page} of {data.pagination.pages}
          </Typography>
          <Button disabled={page >= data.pagination.pages} onClick={() => setPage(page + 1)} sx={{ color: '#9ca3af', textTransform: 'none' }}>Next</Button>
        </Box>
      )}
      ...
      >
        ...
        <MenuItem onClick={() => handleActionClick('delete')} sx={{ '&.MuiMenuItem-root': { color: '#ef4444 !important' } }}>
      ...
      >
        ...
        <DialogContent>
          <Box sx={{ pt: 1 }}>
            <Typography variant="body2" sx={{ mb: 2 }}>{getActionMessage(currentAction)}</Typography>
        ...
        <DialogActions>
          <Button onClick={() => setActionDialogOpen(false)} sx={{ color: '#9ca3af' }}>Cancel</Button>
          <Button
            onClick={handleConfirmAction}
            disabled={actionLoading}
            sx={{ color: currentAction === 'delete' ? '#ef4444' : '#22c55e', fontWeight: 600 }}
          >
      ...
      >
        ...
        <DialogContent>
          <Box sx={{ pt: 1 }}>
            <Typography variant="body2" sx={{ mb: 2 }}>
              Send an admin invitation email. The recipient must have or create an account with this email to accept.
            </Typography>
            <TextField
              fullWidth
              label="Email address"
        ...
        <DialogActions>
          <Button onClick={() => setInviteDialogOpen(false)} sx={{ color: '#9ca3af' }}>Cancel</Button>
          <Button
