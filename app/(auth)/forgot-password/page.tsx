...
const btnSx = {
  ...
  '&:disabled': { color: 'rgba(255, 255, 255, 0.4)', borderColor: 'rgba(255, 255, 255, 0.1)' },
};
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setError('Failed to process the link. Please enter your email manually.');
        setStage('email');
      } finally {
  ...
  return (
    <Box sx={{ minHeight: '90vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #141a16 0%, #1c2420 50%, #141a16 100%)', p: 2 }}>
      <Box sx={{ maxWidth: '420px', width: '100%', backdropFilter: 'blur(20px)', background: 'linear-gradient(135deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.02) 100%)', border: '1px solid rgba(255, 255, 255, 0.1)', borderRadius: '16px', p: 3 }}>
        ...
        {error && (
          <Box sx={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)', color: '#f87171', p: 1.5, borderRadius: '8px', mb: 2, fontSize: '0.85rem' }}>
            {error}
          </Box>
        )}
        ...
        {stage === 'reset' && (
          <form onSubmit={handleResetSubmit}>
            <TextField fullWidth label="New Password" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} margin="dense" sx={textFieldSx} required />
        ...
        {stage === 'done' && (
          <Box sx={{ textAlign: 'center', py: 2 }}>
            <Typography sx={{ color: '#a3e635', fontSize: '1.1rem', fontWeight: 600, mb: 2 }}>
              Your password has been reset.
            </Typography>
            <Typography sx={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.9rem', mb: 3 }}>
              You can now sign in with your new password.
            </Typography>
            <Button href="/login" fullWidth variant="contained" sx={btnSx}>
              Sign In
            </Button>
          </Box>
        )}
