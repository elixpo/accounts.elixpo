  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const lookupRes = await fetch(`/api/auth/verify-by-token?token=${encodeURIComponent(token)}`);
        ...
        if (!lookupRes.ok) {
          if (lookupData.alreadyVerified) {
            ...
            setStatus('error');
            setMessage(lookupData.error || 'Invalid verification link.');
          }
          return;
        }
      ...
      >
        {status === 'loading' && (
          <>
            ...
            <Typography sx={{ color: '#f5f5f4', fontSize: '1.1rem' }}>{message}</Typography>
          </>
        )}
