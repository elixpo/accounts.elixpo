...
import { generateUUID, generateRandomString } from '../../../../src/lib/webcrypto';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    ).bind(generateUUID(), email, session.userId, token, expiresAt).run();
    ...
    const inviter = await db.prepare('SELECT email FROM users WHERE id = ?').bind(session.userId).first() as any;
    const inviterEmail = inviter?.email || 'an admin';
    const recipientName = email.split('@')[0];

    const t = emailTemplates.adminInvite(recipientName, inviterEmail, inviteLink);
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (invite.accepted) {
      return NextResponse.json({ error: 'This invitation has already been accepted', alreadyAccepted: true }, { status: 400 });
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const user = await db.prepare('SELECT id, email FROM users WHERE id = ?').bind(payload.sub).first() as any;
    if (!user) {
