...
export async function PATCH(request: NextRequest) {
    ...
    try {
        const { appId, action } = (await request.json()) as {
            appId?: string;
            action?: string;
        };
        if (!appId || !action)
            return NextResponse.json(
                { error: "appId and action are required" },
                { status: 400 },
            );
        ...
        if (action === "suspend") {
            await db
                .prepare(
                    "UPDATE oauth_clients SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE client_id = ?",
                )
                .bind(appId)
                .run();
            ...
            await db
                .prepare(
                    "UPDATE oauth_clients SET is_active = 1, updated_at = CURRENT_TIMESTAMP WHERE client_id = ?",
                )
                .bind(appId)
                .run();
        } else {
        ...
        return NextResponse.json({
            success: true,
            message: `App ${action === "suspend" ? "suspended" : "activated"}`,
        });
    } catch (error) {
...
export async function DELETE(request: NextRequest) {
    ...
    try {
        ...
        const app = (await db
            .prepare(
                "SELECT name, owner_id FROM oauth_clients WHERE client_id = ?",
            )
            .bind(appId)
            .first()) as any;
        if (!app)
        ...
        await db
            .prepare("DELETE FROM auth_requests WHERE client_id = ?")
            .bind(appId)
            .run();
        await db
            .prepare("DELETE FROM refresh_tokens WHERE client_id = ?")
            .bind(appId)
            .run();
        await db
