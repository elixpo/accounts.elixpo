  ...
  try {
    ...
    if (body.token && !body.code) {
      ...
      if (new Date(record.expires_at) < new Date()) {
        return NextResponse.json({ error: 'This link has expired. Please request a new code.' }, { status: 400 });
      }
      ...
      return NextResponse.json({ code: record.otp_code, email: realEmail });
    }
    ...
    ).bind(user.id, `reset:${email}`, code).first() as any;
    ...
    // Mark token as used
    await db.prepare('UPDATE email_verification_tokens SET is_verified = 1, verified_at = CURRENT_TIMESTAMP WHERE id = ?')
      .bind(record.id).run();
    ...
    await db.prepare("DELETE FROM email_verification_tokens WHERE user_id = ? AND email LIKE 'reset:%'")
      .bind(user.id).run();
