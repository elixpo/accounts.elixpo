...
export async function POST(request: NextRequest) {
  try {
    ...
    const recent = await db
      .prepare('SELECT created_at FROM email_verification_tokens WHERE user_id = ? AND email = ? ORDER BY created_at DESC LIMIT 1')
    ...
    ).bind(tokenId, user.id, `reset:${email}`, otp, verificationToken, expiresAt).run();
    ...
    const t = emailTemplates.passwordResetOtp(recipientName, otp, verifyLink, expiryMinutes);
