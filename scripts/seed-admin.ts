...
if (!ACCOUNT_ID || !API_TOKEN || !DATABASE_ID) {
  console.error('ERROR: CLOUDFLARE_ACCOUNT_ID, CLOUDFLARE_API_TOKEN, and CLOUDFLARE_DATABASE_ID are required.');
  process.exit(1);
}
...
async function query(sql: string, params: (string | number | null)[] = []) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`D1 error: ${err.errors?.[0]?.message || res.statusText}`);
  }
...
async function main() {
  ...
  if (existingUser) {
    if (existingUser.is_admin) {
      console.log(`Admin account already exists and already has admin privileges.`);
