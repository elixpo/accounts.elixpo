...
class SmtpConnection {
    ...
    async connect(config: SmtpConfig): Promise<void> {
        ...
        this.socket = connect({ hostname: config.host, port: config.port }, {
        ...
        if (!config.secure) {
            ...
            if (ehlo2.code !== 250)
                throw new Error(
                    `SMTP EHLO after STARTTLS failed: ${ehlo2.text}`,
                );
        }
        ...
        if (authPlain.code !== 235) {
            ...
            if (authStart.code !== 334)
                throw new Error(
                    `SMTP AUTH failed: PLAIN=${authPlain.text}, LOGIN=${authStart.text}`,
                );
    ...
    private async readResponse(): Promise<{ code: number; text: string }> {
        while (true) {
            ...
            if (done)
                throw new Error(
                    `SMTP connection closed unexpectedly. Buffer: ${this.buffer}`,
                );
