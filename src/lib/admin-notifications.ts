...
export async function getNotificationSettings(
    ...
    const result = (await db
        .prepare(
            `SELECT * FROM admin_notification_settings WHERE id = 'singleton'`,
        )
        .first()) as any;
    return {
        ...
        digest_frequency: (result?.digest_frequency ?? "weekly") as
            | "daily"
            | "weekly",
...
export async function updateNotificationSettings(
    ...
    const fields: string[] = [];
    const values: any[] = [];
    if (settings.email_new_user !== undefined) {
        fields.push("email_new_user = ?");
        values.push(settings.email_new_user ? 1 : 0);
    }
    if (settings.email_new_oauth_app !== undefined) {
        fields.push("email_new_oauth_app = ?");
        values.push(settings.email_new_oauth_app ? 1 : 0);
    }
    if (settings.email_new_api_key !== undefined) {
        fields.push("email_new_api_key = ?");
        values.push(settings.email_new_api_key ? 1 : 0);
    }
    if (settings.email_suspicious_login !== undefined) {
        fields.push("email_suspicious_login = ?");
        values.push(settings.email_suspicious_login ? 1 : 0);
    }
    if (settings.digest_enabled !== undefined) {
        fields.push("digest_enabled = ?");
        values.push(settings.digest_enabled ? 1 : 0);
    }
    if (settings.digest_frequency !== undefined) {
        fields.push("digest_frequency = ?");
        values.push(settings.digest_frequency);
    }
    if (settings.admin_email !== undefined) {
        fields.push("admin_email = ?");
        values.push(settings.admin_email);
    }
    if (fields.length === 0) return;
    fields.push("updated_at = CURRENT_TIMESTAMP");
    await db
        .prepare(
            `UPDATE admin_notification_settings SET ${fields.join(", ")} WHERE id = 'singleton'`,
        )
        .bind(...values)
        .run();
}
