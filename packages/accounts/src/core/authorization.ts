...
function validateRedirectUri(value: string): string {
    ...
    try {
        ...
        throw new AccountsError("configuration_error", "redirectUri must be an absolute URL");
    }
    ...
    if (redirect.protocol !== "https:" && !(local && redirect.protocol === "http:")) {
        throw new AccountsError("configuration_error", "redirectUri must use HTTPS outside localhost");
    }
    if (redirect.username || redirect.password || redirect.hash) {
        throw new AccountsError("configuration_error", "redirectUri cannot contain credentials or a fragment");
    }
...
export function parseAuthorizationCallback(
    ...
    const url =
        callbackUrl instanceof URL ? callbackUrl : new URL(callbackUrl);
    const returnedState = url.searchParams.get("state") || "";
    assertState(expectedState, returnedState);
    const error = url.searchParams.get("error");
    if (error) {
        throw new AccountsError("oauth_error", "Authorization request was rejected", {
...
function validateScopes(scopes: string[]): string[] {
    ...
    if (normalized.length === 0 || normalized.some((scope) => /\s/.test(scope))) {
        throw new AccountsError("configuration_error", "At least one valid OAuth scope is required");
    }
    return normalized;
}
...
export async function createAuthorizationRequest(
    ...
    configuration: Pick<AccountsConfiguration, "clientId" | "redirectUri" | "audience">,
    options: { scopes?: string[]; prompt?: "consent" | "login" | "select_account" } = {},
    ...
    const scopes = validateScopes(options.scopes ?? ["openid", "profile", "email"]);
    ...
    url.searchParams.set("redirect_uri", validateRedirectUri(configuration.redirectUri));
    url.searchParams.set("scope", scopes.join(" "));
    url.searchParams.set("state", state);
    url.searchParams.set("nonce", nonce);
    url.searchParams.set("code_challenge", codeChallenge);
    url.searchParams.set("code_challenge_method", "S256");
    if (configuration.audience) url.searchParams.set("audience", configuration.audience);
